...
def get_covered_distance(
    ...
    try:
        _validate_inputs(tracking_data, player_ids, framerate, acceleration_intervals, start_idx, end_idx)
        ...
        if velocity_intervals:
            ...
            players = _add_covered_distance_interval(players, "velocity", tracking_data, distance_per_frame, velocity_intervals, player_ids)
        if acceleration_intervals:
            acceleration_intervals = _parse_intervals(acceleration_intervals)
            players = _add_covered_distance_interval(players, "acceleration", tracking_data, distance_per_frame, acceleration_intervals, player_ids)
...
def _add_covered_distance_interval(
    ...
    for player_id in player_ids:
        ...
        for min_val, max_val in intervals:
            ...
            players[player_id]['total_distance_' + interval_type].append({f"{min_val}_{max_val}": total_distance})
    return players
...
def _validate_inputs(tracking_data, player_ids, framerate, acceleration_intervals, start_idx, end_idx):
    ...
    for idx in [idx for idx in [start_idx, end_idx] if idx is not None]:
        if not isinstance(idx, int):
            raise TypeError(f"start_idx and end_idx must be integers, not {type(idx).__name__}")
