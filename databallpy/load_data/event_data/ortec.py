...
for ortec_event in ORTEC_EVENTS_MAP.values():
    if "pass" in ortec_event.lower() or "corner" in ortec_event.lower() or "throw" in ortec_event.lower() or "cross" in ortec_event.lower():
    ...
    elif "dribble" in ortec_event.lower() or "duelattackingpartground" in ortec_event.lower():
...
def load_ortec_event_data(
    ...
    event_data.loc[home_mask, "player_name"] = event_data.loc[home_mask, "player_id"].map(metadata.home_players.set_index("id")["full_name"])
    event_data.loc[away_mask, "player_name"] = event_data.loc[away_mask, "player_id"].map(metadata.away_players.set_index("id")["full_name"])
...
def _load_ortec_event_data(event_data_loc: str) -> pd.DataFrame:
    ...
    for event in data["Events"]:
        ...
        result_dict["player_id"].append(annot["PersonId"] if "PersonId" in annot.keys() else MISSING_INT)
        result_dict["team_id"].append(annot["SelectionEditionId"] if "SelectionEditionId" in annot.keys() else MISSING_INT)
        result_dict["type_id"].append(annot["Label"]) if "Label" in annot.keys() else MISSING_INT
        event_name = ORTEC_EVENTS_MAP[annot["Label"]] if "Label" in annot.keys() else "Unknown"
        result_dict["ortec_event"].append(event_name)
        result_dict["start_x"].append(annot["LocationX"] if "LocationX" in annot.keys() else np.nan)
        result_dict["start_y"].append(annot["LocationY"] if "LocationY" in annot.keys() else np.nan)
        result_dict["to_player_id"].append(annot["NextPersonId"] if "NextPersonId" in annot.keys() else MISSING_INT)
        result_dict["to_team_id"].append(annot["NextSelectionEdition"] if "NextSelectionEdition" in annot.keys() else MISSING_INT)
        result_dict["end_x"].append(annot["NextLocationX"] if "NextLocationX" in annot.keys() else np.nan)
        result_dict["end_y"].append(annot["NextLocationY"] if "NextLocationY" in annot.keys() else np.nan)
        if len(annot["Properties"]) > 0:
            result_dict["outcome"].append([0, 1][ORTEC_COMPLETED_PROPERTY_ID in annot["Properties"]])
        else:
